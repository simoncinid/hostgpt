...
export default function ChatWidgetPage() {
  ...
  const downloadHouseRulesPDF = () => {
    ...
    try {
      // Nome del file basato sulla lingua
      ...
      // Crea contenuto HTML per stampa/PDF
      const printContent = `
        <!DOCTYPE html>
        <html>
        <head>
          <meta charset="UTF-8">
          <title>${language === 'IT' ? 'Regole della Casa' : 'House Rules'}</title>
          <style>
            @media print {
              body { margin: 0; }
        ...
        <body>
          <button class="download-btn no-print" onclick="window.print()">${language === 'IT' ? 'Stampa/Salva PDF' : 'Print/Save PDF'}</button>
          <h1>${language === 'IT' ? 'Regole della Casa' : 'House Rules'}</h1>
          <div class="property-name">${chatInfo.property_name}</div>
          <div class="content">${chatInfo.house_rules.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
          <div class="footer">
            ${language === 'IT' ? 'Generato da HostGPT' : 'Generated by HostGPT'} - ${new Date().toLocaleDateString()}
          </div>
        </body>
        </html>
      `
      ...
      if (printWindow) {
        ...
        // Attiva automaticamente la finestra di stampa
        printWindow.onload = () => {
          setTimeout(() => {
            printWindow.print()
          }, 500)
        ...
        toast.success(language === 'IT' ? 'Finestra di stampa aperta' : 'Print window opened')
      } else {
        throw new Error('Popup blocked')
      }
    } catch (error) {
      console.error('Error opening print window:', error)
      toast.error(language === 'IT' ? 'Errore nell\'apertura della stampa' : 'Error opening print window')
    }
  }
